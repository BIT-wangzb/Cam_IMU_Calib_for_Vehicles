...
class IccCamera():
    ...
    def findTimeshiftCameraImuPrior(self, imu, verbose=True):
        print("BIT: Estimating time shift camera to imu:")
        ...
        #---bit modify---
        f = open('/home/wangzb/Videos/2/zed_imu.txt','r')
        line = f.readline()
        imu_time_txt = []
        imu_omega = []
        for line in f.readlines():
            ...
            l = line.split(' ')
            timestamp = float(l[0])
            imu_time_txt.append(timestamp)
            omega_x = float(l[4])
            omega_y = float(l[5])
            omega_z = float(l[6])
            omega = np.array([[omega_x],[omega_y],[omega_z]])
            imu_omega.append(omega)
        
        for i in range(0,len(imu_time_txt)):
            tk = imu_time_txt[i]
            if tk > poseSpline.t_min() and tk < poseSpline.t_max():
                omega_measured = imu_omega[i]
        ...
        #np.argmax(返回数组最大处的索引值)
        discrete_shift = corr.argmax() - (np.size(omega_measured_norm) - 1)
        print(np.size(omega_measured_norm))
        print(discrete_shift)
        
        #get cont. time shift
        times = [im for im in imu_time_txt]
        ...
        pl.plot(t, omega_predicted_norm, label="predicted")
        pl.legend()
        pl.title("Time shift prior camera-imu raw")
    ...
    def initPoseSplineFromCamera(self, splineOrder=6, poseKnotsPerSecond=100, timeOffsetPadding=0.02):
        ...
        # ----bit modify---
        f = open('/home/wangzb/Videos/2/right_pose.txt','r')
        line = f.readline()
        time_txt=[]
        vec_result=[]
        for line in f.readlines():
            line = line.strip(' ')
            l = line.split(' ')
            timestamp = float(l[0])+self.timeshiftCamToImuPrior
            # print(timestamp)
            time_txt.append(timestamp)
            t_x = float(l[1])
            t_y = float(l[2])
            t_z = float(l[3])
            ...
            Rot = r.as_matrix()
            tmp_ = np.array([[0,0,0,1]])
            tem_T = np.concatenate((Rot,t),axis=1)
            T = np.concatenate((tem_T,tmp_),axis=0)
            vec_result.append(T)
        f.close()
        #-------------------------------
        ...
        # ---bit modify---
        times = np.array(time_txt)
        curve = np.matrix([ pose.transformationToCurveValue( np.dot(obs, T_c_b) ) for obs in vec_result]).T
        #--------------
        # 原始
